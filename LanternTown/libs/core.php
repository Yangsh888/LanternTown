...
function parseAvatar(mixed $mail, int $return = 0): ?string
{
    $options = \Typecho\Widget::widget('\Widget\Options');
    $fallback = rtrim((string) $options->themeUrl, '/') . '/assets/img/avatar.svg';
...
function getThumb(object $archive, object $options): string
{
    ...
    return loadThumb(lt_text($archive->content ?? ''), $options);
}

function loadThumb(string $content, object $options): string
{
    $thumbs = lt_lines($options->indexThumbs ?? '');
    if (!empty($thumbs)) {
        $randomThumb = $thumbs[array_rand($thumbs)];
        return lt_safe_url($randomThumb);
    }
...
function lt_render_post_link(array $content, object $item, object $options, string $text): string
{
    $permalink = lt_safe_url(lt_text($content['permalink'] ?? ''));
    $title = lt_text($content['title'] ?? '');
    $thumb = getThumb($item, $options);
...
function theNext(object $widget, object $options): void
{
    try {
        ...
        $sql = $db->select()
            ->from('table.contents')
            ->where('table.contents.created > ?', (int) $widget->created)
